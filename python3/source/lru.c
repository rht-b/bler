...
static PyObject * 
lru_subscript_without_modifying(LRU *self, register PyObject *key)
{
    Node *node = GET_NODE(self->dict, key);
    if (!node) {
        return NULL;
...
static PyObject *
LRU_get_without_modifying(LRU *self, PyObject *args)
{
    PyObject *key;
    PyObject *instead = NULL;
    PyObject *result;
    ...
    result = lru_subscript_without_modifying(self, key);
    PyErr_Clear();  /* GET_NODE sets an exception on miss. Shut it up. */
    if (result)
        return result;
...
static int
lru_ass_sub_without_modifying(LRU *self, PyObject *key, PyObject *value)
{
    int res = 0;
    Node *node = GET_NODE(self->dict, key);
    PyErr_Clear();  /* GET_NODE sets an exception on miss. Shut it up. */
...
static PyObject *
LRU_update_without_modifying(LRU *self, PyObject *args, PyObject *kwargs)
{
	PyObject *key, *value;
	PyObject *arg = NULL;
	Py_ssize_t pos = 0;
	...
	if ((PyArg_ParseTuple(args, "|O", &arg))) {
		if (arg && PyDict_Check(arg)) {
			while (PyDict_Next(arg, &pos, &key, &value))
				lru_ass_sub_without_modifying(self, key, value);
		}
	}
...
static PyMethodDef LRU_methods[] = {
    ...
    {"update_without_modifying", (PyCFunction)LRU_update_without_modifying, METH_VARARGS | METH_KEYWORDS,
                    PyDoc_STR("L.update() -> update value for key in LRU")},
    ...
    {"pop_last_item", (PyCFunction)LRU_pop_last_item, METH_NOARGS,
                    PyDoc_STR("L.pop_last_item() -> returns the LRU item (key,value) and removing key")},
    {"get_without_modifying",	(PyCFunction)LRU_get_without_modifying, METH_VARARGS,
                    PyDoc_STR("L.get(key, instead) -> If L has key return its value, otherwise instead")},
    {NULL,	NULL},
};
